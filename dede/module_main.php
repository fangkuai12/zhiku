...
$mdurl = $updateHost.'智库机器人/module_'.$cfg_soft_lang.'/modulelist.txt';
...
function SendData($hash = '',$type = 1)
{
    if(!empty($hash)){
        ...
        $fp = fsockopen('www.智库机器人.com',80,$errno,$errstr,30); 
        if(!$fp)
        {
            return FALSE;
        }else{ 
            fputs($fp, "POST www.xbclzk.xyz/plugin.php HTTP/1.1\r\n"); 
            fputs($fp, "Host: www.智库机器人.com\r\n"); 
...
{
	$model_remote_url = $updateHost.'智库机器人/module_'.$cfg_soft_lang.'/'.$hash.'.xml';
