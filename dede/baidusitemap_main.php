...
if($dopost=='auth'){
    if ( empty($sign) )
    {
        ...
        $sigurl="http://baidu.api.智库机器人.com/index.php?siteurl=".urlencode($siteurl);
        ...
        if($data['status']==0){
            ...
            if($output['status']==0){
                ...
                ShowMsg("提交百度索引失败，无法校验本地密钥！远程接口服务器无法正常获取到您的站点文件！ <a href='www.xbclzk.xyz/addons/baidusitemap/#help' target='_blank'>点击获取更多帮助</a>","javascript:;");
                exit();
            }
        }
    } else {
...
} elseif ( $dopost=='searchbox2' || $dopost=='searchpage2' || $dopost=='income2' || $dopost=='report2')
{
    ...
    $str = <<<EOT
<script type="text/javascript" src="http://baidu.api.智库机器人.com/assets/js/jquery.min.js"></script>
<html>
	<head>
...
} elseif ( $dopost=='bind_site_id' )
{
    ...
    if($data['status']==0){
        ...
        if($output['status']==0){
            ...
            ShowMsg("无法校验本地密钥，远程接口服务器无法正常获取到您的站点文件！ <a href='www.xbclzk.xyz/addons/baidusitemap/#help' target='_blank'>点击获取更多帮助</a>","javascript:;");
            exit();
        }
    }
} elseif ( $dopost=='ping1' )
{
    $sigurl="http://baidu.api.智库机器人.com/index.php";
    ...
    if ( $output['status']==1 )
    {
        ...
        ShowMsg("无法连接：您的服务器无法正常连接'http://baidu.api.智库机器人.com'，请确保服务器环境支持远程获取文件。<a href='www.xbclzk.xyz/addons/baidusitemap/#help' target='_blank'>点击获取更多帮助</a>",'javascript:;');
        exit();
    }
} elseif ( $dopost=='ping2' )
{
    ...
    if ( $output['status']==1 )
    {
        ...
        ShowMsg("无法连接：您的服务器无法正常连接'http://zhanzhang.baidu.com/api'，请确保服务器环境支持远程获取文件。<a href='www.xbclzk.xyz/addons/baidusitemap/#help' target='_blank'>点击获取更多帮助</a>",'javascript:;');
        exit();
    }
} elseif ( $dopost=='bind' )
{
				...
				<table width="98%" border="0" cellspacing="1" cellpadding="1">
					<tbody>
						<tr>
							...
							<td width="84%" style="text-align:left;"><span style='color:black'><iframe name='stafrm' src='http://baidu.api.智库机器人.com/index.php?c=welcome&m=new_ver&ver={$ver}&siteurl={$siteurl2}&setupmaxaid={$setupmaxaid}' frameborder='0' id='stafrm' width='98%' height='22'></iframe></span>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td width="84%" style="text-align:left;"><span style='color:black'>http://baidu.api.智库机器人.com</span> <a href='?dopost=ping1' style='color:blue'><u>[检查通信]</u></a>
							</td>
						</tr>
						<tr>
