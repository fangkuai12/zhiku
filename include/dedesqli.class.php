...
class DedeSqli
{
    ...
    function Open($pconnect=FALSE)
    {
        ...
        else
        {
            ...
            mysqli_errno($this->linkID) != 0 && $this->DisplayError('智库机器人错误警告： 链接('.$this->pconnect.') 到MySQL发生错误');
        ...
        if(!$this->linkID)
        {
            $this->DisplayError("智库机器人错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错！</font>");
            exit();
        }
		$this->isInit = TRUE;
...
EOT;
    ...
    function DisplayError($msg)
    {
		...
		{
			$emsg = '';
			$emsg .= "<div><h3>智库机器人 Error Warning!</h3>\r\n";
			$emsg .= "<div><a href='http://bbs.智库机器人.com' target='_blank' style='color:red'>Technical Support: http://bbs.智库机器人.com</a></div>";
